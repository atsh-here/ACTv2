...
impl BatchedEqualityProof {
    ...
    pub fn prove(
        ...
        let r2_point = G1Projective::from(bls_h1_affine()) * k_mod_r + G1Projective::from(bls_g1_affine()) * u2;

        let mut hasher = Sha256::new();
        hasher.update(b"cross_curve_batched_proof_v1");
        hasher.update(r1_point.compress().as_bytes());
        hasher.update(r2_point.to_affine().to_compressed().as_ref());
        let c_hash = hasher.finalize();
        let c = BigUint::from_bytes_be(&c_hash[0..16]);
    ...
    pub fn verify(
        ...
        let mut hasher = Sha256::new();
        hasher.update(b"cross_curve_batched_proof_v1");
        hasher.update(self.r1_comb.compress().as_bytes());
        hasher.update(self.r2_comb.to_affine().to_compressed().as_ref());
        let c_hash = hasher.finalize();
        let c = BigUint::from_bytes_be(&c_hash[0..16]);
        ...
        let lhs_bls = G1Projective::from(bls_h1_affine()) * z_r + G1Projective::from(bls_g1_affine()) * self.z_r2;
